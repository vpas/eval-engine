"""ClickHouse analytics store — the REAL backend (SCHEMA §2).

Same interface as ``analytics`` (DuckDB). Uses the production engine choice:
ReplacingMergeTree(attempt) keyed on (eval_id,target_id,run_id,sample_id), monthly partitions,
12-month TTL. Connection via ``EVAL_ENGINE_CH_*`` env (defaults to local docker ClickHouse).
"""
